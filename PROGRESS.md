...
## 2026-08-15 — Step 8b: Frontend UI

- `lib/api.ts`: typed client functions for every endpoint (papers, upload, query, eval logs, rerank log, golden eval run).
- `components/UploadPanel.tsx` — PDF upload + live papers list (sidebar).
- `components/ChatWindow.tsx` + `MessageBubble.tsx` + `SourcesPanel.tsx` — chat interface; each answer shows expandable citation chips (`paper · page · section`) and, for multi-part questions, the decomposed sub-queries.
- `components/EvalBadge.tsx` — since RAGAS scoring runs in a backend background task (not inline in the `/api/query` response, see above), this polls `GET /api/eval/logs` briefly (up to ~60s, 5s intervals) until the matching `query_id` shows up with scores, then renders faithfulness/relevancy chips. Gives up gracefully (renders nothing) if scoring takes unusually long, rather than polling forever.
- `app/eval/page.tsx` — dashboard: golden-set eval trigger + aggregate scores, recent-query score table, and a re-ranking before/after panel (both bi-encoder and cross-encoder top-5 shown side by side per query, with a rank-changes count) pulled straight from the JSONL logs.
- **Lint fix**: Next.js 16's bundled `eslint-config-next` includes a newer React Compiler rule (`react-hooks/set-state-in-effect`) that flagged the initial `UploadPanel` fetch-on-mount pattern. Fixed by using the cancellation-guard pattern (matching what `EvalBadge` already used) instead of a bare fire-and-forget async call in the effect body. `npx tsc --noEmit` and `npx eslint .` both clean.
...
## 2026-08-15/16 — Step 10: Deployment attempt, Render OOM, decision to ship Option B

Attempted a live hosted deployment (frontend on Vercel, backend on Render) per the original plan and the user's initial preference.

- Installed the GitHub CLI-equivalent flow for two more services: installed the Vercel CLI (`npm install -g vercel`, no login completed since deployment was abandoned before reaching the frontend step) and drove Render's REST API directly using an API key the user generated (same pattern as the Anthropic/GitHub key exchanges).
- Wrote a root-level `Dockerfile` (bakes in both HF models at build time, fetches the seed PDFs at build time, runs `uvicorn` respecting Render's `$PORT`) and a `render.yaml` Blueprint spec.
- **Debugging detour**: the first three deploy attempts all failed with `failed to read dockerfile: open Dockerfile: no such file or directory`, despite trying three different `dockerfilePath`/`dockerContext`/`rootDir` combinations against Render's direct `POST /v1/services` API. Initially suspected a Render API quirk (the direct-creation API possibly not honoring custom Dockerfile-path fields the way the dashboard UI or Blueprint flow do — verified via `WebFetch` against Render's actual docs page, which confirms a `Dockerfile Path` field exists and behaves as documented for the dashboard/Blueprint path). Eventually found the real cause was self-inflicted: after relocating the Dockerfile from `backend/Dockerfile` to a root-level `Dockerfile` (to sidestep the path-resolution uncertainty entirely), the already-created Render service's `envSpecificDetails.dockerfilePath` was never updated and still pointed at the now-deleted `backend/Dockerfile` -- a stale-config bug on our side, not a platform bug. Fixed by PATCHing the service's `dockerfilePath`/`dockerContext` to match the new root location.
- With the config corrected, the Docker build succeeded end-to-end (~2 min: pip install, bake in `sentence-transformers`/`CrossEncoder` weights, fetch seed PDFs, export image). **The container then crashed on startup with `Out of memory (used over 512Mi)`** -- confirmed via Render's logs API, occurring during the FastAPI `lifespan` startup (loading the embedding model + building the FAISS index), before any request was served. This is a genuine, well-known constraint: PyTorch + `sentence-transformers` routinely need 1GB+ RAM just to load, regardless of how small the actual embedding model is -- Render's free tier caps at 512MB with no way to raise it without upgrading to a paid plan.
- Presented the tradeoff to the user directly (upgrade to a paid tier / attempt a lighter ONNX-based embedding backend under time pressure / ship as Option B repo-only) rather than picking one unilaterally, since it's a real cost-vs-scope decision. **User chose Option B (repo-only submission)** -- no further hosting cost, no risk to the fully-verified working pipeline from a last-minute architecture change.
- Cleaned up: deleted the Render service via the API. Kept `Dockerfile` and `render.yaml` in the repo (they're correct and would work on any host with ≥1GB RAM) with an honest README section explaining exactly what was tried, what failed, and why -- per the submission guidelines' explicit instruction that "overclaiming is penalized harder than an honestly scoped-down submission."
- **Final verification**: ran a genuine clean-machine test -- fresh `git clone` into a scratch directory, fresh Python venv, `pip install -r requirements.txt` from a clean slate, `python scripts/fetch_papers.py`, `uvicorn` boot, fresh `npm install`, `npm run dev`, then the full Playwright browser flow (home page → ask question → citations render → eval badge resolves → eval dashboard loads) against that clean-clone instance. **Zero console errors, matches documented `/health` output exactly.** This is the strongest evidence available that a reviewer following only the README will succeed within the stated setup time.
- ✅ Step 10 checkpoint met (repo-only path): the project is fully built, verified end-to-end from a genuinely clean clone, and honestly documented including the deployment attempt and its outcome.
